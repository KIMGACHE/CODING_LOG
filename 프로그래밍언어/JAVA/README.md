...
## 인터페이스
일반 매서드는 없이 전부 추상 매서드로만 구성되어 있다. (public abstract) <br>
일반 멤버 변수는 없고 public static final로 선언된 상수로만 구성되어 있다. <br>
직접 객체를 생성하는 것은 불가능하고 implements 키워드를 사용하여 다른 클래스로 구현할 때 객체를 생성할 수 있다. <br>
implements를 사용한 클래스는 반드시 추상 매서드를 재정의해야만 한다. <br>
<br>

**인터페이스를 사용하는 이유** <br>
1. 낮은 결합도를 유지
2. 설계단계에서 인터페이스를 통해 정의한 내용을 코딩단계에서 클래스로 구현되도록 강제할 수 있다.
